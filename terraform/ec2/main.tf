...
variable "dns_service_ip" {
  description = "Cluster DNS the flex nodes use as --cluster-dns. Points at the flex-node-system chart's system-node CoreDNS VIP (10.0.0.11), NOT the cluster-wide kube-dns (10.0.0.10) — kube-dns endpoints spread onto Karpenter nodes whose NICs lack IP-forwarding and are unreachable from a flex node. Per-node kubelet flag, so the rest of the cluster is unaffected."
  type        = string
  default     = "10.0.0.11"
}
